    ...
    try:
        ...
        if dob_str:
            ...
            if age_val is not None:
                profile_results_dict["current_client_age"] = int(age_val)
        ...
        if profile_results_dict["current_client_age"] is not None:
            ytr_val = run_mcp_and_extract(
                ...
                {
                    "start_value": profile_results_dict["current_client_age"],
                    "end_value": retirement_age,
        ...
        if assets_list_raw:
            ...
            if profile_results_dict["years_to_retirement"] is not None and profile_results_dict["equity_exposure_status"] is not None:
                risk_val = run_mcp_and_extract(
                    "assess_client_risk_appetite",
                    {
                        "years_to_retirement": profile_results_dict["years_to_retirement"],
        ...
        for key, value in family_info.items():
            # only match keys like 'first_child_ahira_year_of_birth'
            m = re.match(r'^(first|second|third|fourth|fifth)_child_([^_]+)_year_of_birth$', key, flags=re.IGNORECASE)
            if not m:
                continue
            label = m.group(1).title()
            name = m.group(2).replace('_', ' ').title()
            child_identifier = f"{label} {name}"
            try:
                yob = int(value)
            except:
                continue
            child_dob = f"{yob}-01-01"
            ...
            temp_child_profiles.append({
                ...
                "ug_horizon_classification": ug_horizon,
        ...
        # Validate and return
        validated = ClientProfileCalculations(**profile_results_dict)
        return validated.model_dump_json(indent=2)

    except Exception as e:
        import traceback
        return json.dumps({
            "error": f"COMPREHENSIVE_PROFILE_CALCULATOR_TOOL_ERROR: {type(e).__name__} - {e}",
...
def define_education_type(
    ...
    """
    For each UG/PG goal in future_plans (keys matching undergraduation_of_* or post_graduation_of_*),
    decide Domestic vs. International. Works for any number of children.
    If no educational goals are found, returns an empty dict.
    """
    recommendations: Dict[str, str] = {}
    # pattern to match both UG and PG keys
    pattern = re.compile(r'^(undergraduation_of_|post_graduation_of_)(.+)$', re.IGNORECASE)

    for key, plan in future_plans.items():
        ...
        stage_prefix, child_name = m.groups()
        years_left = plan.get("time_years", float("inf"))
        # deprioritize PG if there's a retirement gap
        is_pg = stage_prefix.lower().startswith("post_graduation")
        deprio_pg = is_pg and retirement_gap > 0

        # choose based on horizon and thresholds
        if years_left < 5:
            choice = "International (US/UK)" if total_assets >= 2e7 else "Domestic"
        else:
            choice = "International (US/UK)" if net_worth >= 2e7 else "Domestic"

        if deprio_pg:
            choice = "Domestic (PG deprioritized due to retirement gap)"

        recommendations[key] = choice

    # if no educational keys were matched, return empty dict
    return recommendations



def choose_cost_base(
    future_plans: Dict[str, Any],
    edu_type_map: Dict[str, str]
) -> Dict[str, Dict[str, Any]]:
    """
    For each education goal (keys undergraduation_of_* or post_graduation_of_*) in future_plans,
    compute the projected future cost using:
      - International inflation = 7% (0.07)
      - Domestic inflation    = 9.5% (0.095)
    Returns a dict keyed by the same future_plans keys, each containing:
      {
        "education_type": str,
    ...
    for key, plan in future_plans.items():
        if not pattern.match(key) or not isinstance(plan, dict):
            continue

        years = plan.get("time_years")
        cost_today = plan.get("amount_required_today")
        if years is None or cost_today is None:
            # skip any malformed entry
            continue

        # look up domestic vs. international
        ed_type = edu_type_map.get(key, "Domestic")
        if "international" in ed_type.lower():
            inflation = 0.07
        else:
            inflation = 0.095

        # future cost formula
        projected = cost_today * ((1 + inflation) ** years)
...
def evaluate_existing_education_savings(
    future_plans: dict,
    cost_bases: dict,
    annual_roi: float = 0.12
) -> dict:
    """
    For each education goal in future_plans, compute:
      - net_future_value of monthly contributions (annuity FV at annual_roi)
      - additional_savings_required = projected_cost - existing_amount - net_future_value

    Args:
        future_plans: dict of goals, each with keys:
          'time_years', 'monthly_contribution', 'amount_available_today'
        cost_bases: dict mapping goal_key -> {'projected_cost': float}
        annual_roi: float, annual return rate for savings (e.g. 0.12)

    Returns:
        dict mapping each edu goal key to a dict with:
          'existing_amount',
          'monthly_contribution',
          'time_months',
          'net_future_value',
          'projected_cost',
          'additional_savings_required'
    """
    import re

    results = {}
    # pattern matches both undergrad and postgrad keys
    edu_pattern = re.compile(r'^(undergraduation_of_|post_graduation_of_).+', re.IGNORECASE)

    for key, plan in future_plans.items():
        if not edu_pattern.match(key) or not isinstance(plan, dict):
            continue

        # extract parameters
        years = plan.get("time_years", 0)
        months = int(years * 12)
        mthly = plan.get("monthly_contribution", 0) or 0
        existing = plan.get("amount_available_today", 0) or 0

        # compute future value of an annuity
        if months > 0 and mthly > 0:
            r = annual_roi / 12.0
            fv = mthly * (((1 + r) ** months - 1) / r)
        else:
            fv = 0.0

        # lookup projected cost
        projected = cost_bases.get(key, {}).get("projected_cost", 0.0)

        # additional savings needed
        additional = projected - existing - fv
        additional = round(max(additional, 0.0), 2)

        results[key] = {
            "existing_amount": round(existing, 2),
            "monthly_contribution": round(mthly, 2),
            "time_months": months,
            "net_future_value": round(fv, 2),
            "projected_cost": round(projected, 2),
            "additional_savings_required": additional
        }
...
def retirement_cross_check(
    edu_type_map: Dict[str, str],
    cost_bases: Dict[str, Dict[str, Any]],
    savings_plan_fv: Dict[str, Dict[str, Any]],
    retirement_data: Dict[str, Any]
) -> Dict[str, Any]:
    """
    Adjust education goals if retirement is not feasible.
    - If retirement_data['feasible'] is False:
       • mark any International PG as 'Aspirational'
       • reassign priority for UG + retirement
       • compute additional savings = projected_cost - net_savings
    """
    gap = retirement_data.get("gap", 0)
    adjusted = {}

    for key, ed_type in edu_type_map.items():
        # only PG keys
        if not key.lower().startswith("post_graduation"):
            continue

        entry = {
            "education_type": ed_type,
            "original_projected_cost": cost_bases[key]["projected_cost"],
            "savings_fv": savings_plan_fv.get(key, {}).get("net_future_value", 0.0),
            "additional_savings_needed": savings_plan_fv.get(key, {}).get("additional_savings_required", 0.0)
        }

        if gap > 0 and "international" in ed_type.lower():
            entry["status"] = "Aspirational"
        else:
            entry["status"] = "Active"

        # compute extra needed
        # needed = entry["original_projected_cost"] - entry["savings_fv"]
        # entry["additional_savings_needed"] = round(max(needed, 0.0), 2)

        adjusted[key] = entry

    return {
        "retirement_feasible": retirement_data.get("feasible", True),
        "retirement_gap": gap,
        "education_adjustments": adjusted
    }


@tool("educational_planner_tool")
def educational_planner_tool(client_data: dict, client_retirement_data:dict):
    """
    Plans education funding strategy based on client’s financial and retirement data.

    Steps:
    1. Classifies each UG/PG goal as Domestic or International based on asset levels, net worth, and retirement gap.
    2. Estimates future education costs using inflation-adjusted projections.
    3. Evaluates existing savings (lump sum + SIP) and calculates additional savings needed.
    4. Cross-checks with retirement feasibility; deprioritizes PG if there's a gap.

    Args:
        client_data (dict): Includes future education goals and asset/liability details.
        client_retirement_data (dict): Contains retirement gap, corpus needs, and age info.

    Returns:
        dict: Education plan with type, savings evaluation, and retirement-adjusted priorities.
              Returns "no_education_plans" if no education goals are found.
    """
    ed_type_map = define_education_type(
        client_data["future_plans"],
        total_assets=client_data["asset_liability_analysis"]["total_asset_value_numeric"],
        net_worth=client_data["asset_liability_analysis"]["net_worth_numeric"],
        retirement_gap=client_retirement_data["gap"]
    )
    
    if ed_type_map=={}:
        return "no_education_plans"
    
    cost_bases = choose_cost_base(future_plans=client_data["future_plans"],
                                  edu_type_map=ed_type_map)
    
    savings_eval = evaluate_existing_education_savings(client_data["future_plans"],
                                                       cost_bases=cost_bases)
    
    cross_check = retirement_cross_check(
        edu_type_map=ed_type_map,
        cost_bases=cost_bases,
        savings_plan_fv=savings_eval,
        retirement_data=client_retirement_data
    )
    
    return {"education_type":ed_type_map, "savings_eval":savings_eval,
